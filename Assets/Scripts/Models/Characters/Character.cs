...
namespace Assets.Scripts.Models.Characters
{
    public class Character
    {
        ...
        public IReadOnlyDictionary<Type, EquipmentBase> EquippedItems => equippedItems;

        // Thuộc tính tính toán chỉ số
        public int HP => Mathf.RoundToInt(BaseHP * (1 + GetTotalBonusPercent(StatType.HP)) + GetTotalFlat(StatType.HP));
        public int ATK => Mathf.RoundToInt(BaseATK * (1 + GetTotalBonusPercent(StatType.ATK)) + GetTotalFlat(StatType.ATK));
        public int Armor => Mathf.RoundToInt(BaseArmor * (1 + GetTotalBonusPercent(StatType.Armor)) + GetTotalFlat(StatType.Armor));
        public int Regen => Mathf.RoundToInt(BaseRegen * (1 + GetTotalBonusPercent(StatType.Regen)) + GetTotalFlat(StatType.Regen));
        public float Speed => BaseSpeed * (1 + GetTotalBonusPercent(StatType.Speed)) + GetTotalFlat(StatType.Speed);
        public float Dodge => (BaseDodge + GetTotalBonusPercent(StatType.Dodge)) * 100f;
